...
def pla_train(X, y, max_epochs=1000, alpha=0.1, shuffle=True):
    ...
    w = np.array([0.0, 1.0, 0.5])  # initialize weight vector [w0, w1, w2]
...
if __name__ == "__main__":
    X, y0 = make_blobs(n_samples=200, centers=2, n_features=2, cluster_std=1.8)
    y = np.where(y0 == 0, -1, +1)  

    #call and train algorithm
    w, n_updates, epochs_run = pla_train(X, y, max_epochs=1000, alpha=0.1, shuffle=True)
